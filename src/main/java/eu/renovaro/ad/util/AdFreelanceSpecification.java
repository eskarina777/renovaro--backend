...
public class AdFreelanceSpecification {
    ...
    public static Specification<Ad> matches(AdFilterRequest request, Long subcategoryId) {
        return (root, query, cb) -> {
            ...
            predicates.add(cb.equal(serviceType.get("serviceTypeCode"), ServiceTypeCode.FREELANCE));
            ...
            if (request != null) {
                ...
                Expression<BigDecimal> basicPrice = basicPackagePrice(root, query, cb);
                Expression<BigDecimal> priceForFilter = cb.coalesce(basicPrice, new BigDecimal("999999999"));
    ...
    private static Expression<BigDecimal> basicPackagePrice(Root<Ad> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
